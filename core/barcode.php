...
if(isset($_GET["price"])) $price="$".number_format(floatval($_GET["price"]),2);
...
function Barcode39 ($barcode, $width, $height, $quality, $format, $text)
{
    ...
    or die ("Cannot Initialize new GD image stream");
        ...
        if (isset($price)) $BarcodeFull .= " ". $price;
...
{
$newheight = intval($_GET['height']) - 15;
//$clientname = $_GET['fname'] . " " . $_GET['lname'] ;
$text_color = imagecolorallocate($im, 0, 0, 0);
       //imagestring($im, 2, 0, $newheight,  $clientname, $text_color);
        switch ($format)
        {
                case "JPEG":
